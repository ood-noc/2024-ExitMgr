...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="People Counting System with Centroid Tracking"
    )
    parser.add_argument(
        "--debug", "-d", action="store_true", help="デバッグモード（映像を表示）"
    )
    ...
    # カメラのオートフォーカスを無効にする
    cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)  # 0で無効、1で有効
    ...
    while True:
        ...
        if elapsed_time >= 10:
            print(
                f"Entry: {entry_count}, Exit: {exit_count}, Inside: {current_inside}, FPS: {fps:.2f}"
            )
            # スプレッドシートに記録
            manager.append_row_with_timestamp(
                [device_id, entry_count, exit_count, current_inside]
            )
